...
export function rankMeetups(interests: MemberInterest[], candidates: MeetupRankingCandidate[]): { meetupId: string; reasons: string[] }[] {
  return candidates.map((candidate) => {
    ...
    const reasons = [overlap.length ? `Relevant Interests: ${overlap.map((interest) => interest.name).join(", ")}.` : "An open Meetup coming up.",
      `${candidate.connectionCount} recorded Connections with Participants.`];
    return { ...candidate, score: overlap.length, reasons };
