...
import { reportBounds, reportPeriod, reportSelection } from "./reports";
...
export async function memberReport(db: Queryable, actor: Actor, memberId: string, input: ReportPeriod, now: Date, tableId?: string): Promise<Report> {
  ...
  const history = selected("member-counts") ? await readAttendanceHistory(db, person, now, range) : [];
  const occurrences = selected("member-counts") ? await db.select({ id: gatherings.id, kind: gatherings.kind, hostMemberId: gatherings.hostMemberId, membership: gatheringMembers.status }).from(gatherings)
  ...
  const connections = selected("member-connections") ? await readConnections(db, person, range) : [];
  const interests = selected("member-interests") ? await memberInterestsFor(db, actor.organisationId, [memberId]) : [];
  const posts = selected("member-availability") ? await db.select({ activity: activities.name, start: availabilities.startsAt, end: availabilities.endsAt, site: sites.name }).from(availabilities)
  ...
  const [flagCounts] = selected("member-flags") ? await db.select({
