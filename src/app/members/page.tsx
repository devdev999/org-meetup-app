...
import { Avatar, PageHeading } from "../_components/ui";
...
export default async function MembersPage({
  ...
  const interest =
    typeof parameters.interest === "string" ? parameters.interest.trim() : "";
  const department =
    typeof parameters.department === "string"
      ? parameters.department.trim()
      : "";
  const site =
    typeof parameters.site === "string" ? parameters.site.trim() : "";
  ...
  return (
    <main className="directory-page">
      <PageHeading
        title="Find Members"
        description="Browse Members in your Organisation by Interest, Department and Site."
      />
      <form action="/members" className="directory-filters">
        <label>
          Interest
          <input
            name="interest"
          ...
          <datalist id="interest-catalog">
            {catalog.map((item) => (
              <option key={item.interestId} value={item.name}>
                {item.kind === "skill" ? "Skill" : "Hobby"}
              </option>
            ))}
          </datalist>
        </label>
        <label>
          ...
          <select name="department" defaultValue={department}>
            ...
            {department && !choices.departments.includes(department) && (
        ...
        <label>
          ...
          <select name="site" defaultValue={site}>
            ...
            {site && !choices.sites.includes(site) && (
        ...
        <ul className="member-list">
          {members.map((person) => (
            <li key={person.memberId}>
              ...
              <h2>
                <Link href={`/members/${person.memberId}`}>{person.name}</Link>
              </h2>
              <dl>
                <dt>Department</dt>
                <dd>{person.department ?? "Not set"}</dd>
                <dt>Site</dt>
                <dd>{person.site ?? "Not set"}</dd>
              </dl>
