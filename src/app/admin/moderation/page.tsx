...
export default async function ModerationPage({ searchParams }: { searchParams: Promise<{ state?: string | string[] }> }) {
  ...
  return <section>
    ...
    {roster.map((member) => <article className="notice" key={member.memberId}>
      <h4>{member.name}</h4>
    ...
    {occurrences.length === 0 ? <p>No upcoming occurrences.</p> : occurrences.map((occurrence) => <article className="notice" key={occurrence.id}>
