...
export default async function AdminAttendancePage({
  ...
  if (memberId && !roster.some((member) => member.memberId === memberId))
    notFound();
    ...
    <>
      ...
      <section>
        ...
        <form>
          <label>
            Member
            <select name="memberId" defaultValue={memberId ?? ""} required>
        ...
        {memberId && (
          <p>
            <a
              href={`/admin/attendance/export?${new URLSearchParams({ memberId })}`}
              download
            >
              Export Member Attendance as CSV
            </a>
          </p>
        )}
        {history &&
          (history.length === 0 ? (
            <p>No past occurrences for this Member.</p>
          ) : (
            <ul className="meetup-list">
              {history.map((occurrence) => (
                <li key={occurrence.id}>
                  <PastOccurrence occurrence={occurrence} />
                  <p>Attendance: {attendanceLabels[occurrence.outcome]}.</p>
                </li>
              ))}
            </ul>
          ))}
      ...
      <section>
        ...
        <p>
          <a href="/admin/attendance/export" download>
        ...
        {ratings.length === 0 ? (
          ...
          <div
            ...
            tabIndex={0}
            role="region"
            aria-label="Ratings by Activity"
          >
            <table>
              <thead>
                <tr>
                  <th>Activity</th>
                  <th>Ratings</th>
                  <th>Average out of five</th>
                </tr>
              </thead>
              <tbody>
                {ratings.map((rating) => (
                  <tr key={rating.activity.id}>
                    <td>{rating.activity.name}</td>
                    <td>{rating.ratingCount}</td>
