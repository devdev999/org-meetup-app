...
const SIGN_IN_FAILURES: Record<SignInFailure, string> = {
  "inactive-member":
    "Your access has ended or been suspended. Contact your Organisation Admin.",
  expired: "That sign-in took too long. Please start again.",
  rejected:
    "Your Organisation's login did not accept that sign-in. Please start again.",
  "unknown-organisation": "That Organisation is not set up on this platform.",
  "not-configured":
    "Your Organisation's sign-in is awaiting setup. Contact your Organisation Admin.",
  "no-email":
  ...
  "no-pending":
    "We could not match that login to a sign-in started in this browser. Please start again.",
};
  ...
  return (
    <main className="auth-page">
      <div className="auth-visual">
        <img
          src="/atrium/coffee.webp"
          alt="Coffee on a table, an illustrative Activity photograph"
          width="900"
          height="900"
          fetchPriority="high"
        />
        <div>
          ...
          <p>
            Meet people across your Organisation through shared Interests and
            time together.
          </p>
        </div>
      </div>
        ...
        {options.length === 0 ? (
          ...
          <>
            ...
            <ul className="options">
              {options.map((option) => (
                <li key={option.slug}>
                  <a href={`/sign-in/${encodeURIComponent(option.slug)}`}>
                    {option.name}
                  </a>
                </li>
              ))}
