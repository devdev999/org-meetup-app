...
export function ReportTables({ report, memberId }: { report: Report; memberId?: string }) {
  return <>{report.tables.map((table) => {
    const query = new URLSearchParams({ ...report.period, table: table.id, ...(memberId ? { memberId } : {}) });
    return <section key={table.id} className="report-section">
      <h3 id={table.id}>{table.title}</h3>
      <p>{table.basis}</p>
      <p><a href={`/admin/reports/export?${query}`} download>Export {table.title} as CSV</a></p>
