...
Organisation Admins can also open Scout from Administration to ask about existing duplicate Interest proposals, Seeks with no Shares, and report figures. These three tools use the normal merge queue and aggregate reports for the asker's Organisation. Platform Admin status alone does not grant them. Scout links to the merge queue, Interest list and reports; it cannot generate proposals, approve merges or export reports. Aggregate reads follow the dashboard's audit rules, and successful questions record ordinary Member activity.

Report questions accept inclusive dates in the deployment time zone and default to the first day of the current month through today, like the dashboard. Each returned table retains its columns, figures, population and date basis. Scout receives up to twenty rows per table and the full row count. It must not treat the sum of limited rows as an Organisation total. Duplicate results include up to twenty complete proposals with the full proposal count.
